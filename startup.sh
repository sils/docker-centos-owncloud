...
if [ -e /var/www/html/owncloud/INSTALLED ]; then
  ...
  openssl req -nodes -x509 -newkey rsa:4096 -keyout /etc/httpd/conf.d/server.pem -out /etc/httpd/conf.d/server.crt -days 1825 -subj "/C=$SSL_COUNTRY/ST=$SSL_STATE/L=$SSL_LOCALITY/O=$SSL_ORGANISATION/OU=$SSL_ORGANISATION_UNIT/CN=$FQDN"
